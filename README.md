...
Розмови зберігаються в таблиці `public.ai_chats` (створюється тим самим `schema.sql`), прив'язані до конкретного спеціаліста (`owner_id`/`owner_email`) — RLS дозволяє кожному бачити й редагувати лише власні збережені чати, тож вони доступні з будь-якого пристрою під тим самим акаунтом і не плутаються між різними спеціалістами.
